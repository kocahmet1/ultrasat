/**
 * Replace the two Reading & Writing modules of "Exam 6" (Practice Test 6) with
 * the 27+27 questions authored in scripts/data/practiceTest6RW.json.
 *
 ...
 *
 * Usage:
 *   node scripts/replaceExam6RW.js --dry-run     # show the plan, write nothing
 *   node scripts/replaceExam6RW.js               # back up, create, and repoint
 *   node scripts/replaceExam6RW.js --rollback scripts/backups/<file>.json
 */
...
async function main() {
  ...
  if (!rw[1] || !rw[2]) throw new Error('Could not find R&W modules 1 and 2 on Exam 6');
  ...
  if (!DRY_RUN) {
    ...
    const bpath = path.join(dir, `exam6_rw_backup_${Date.now()}.json`);
  ...
  console.log(DRY_RUN ? '[DRY RUN] no changes written' : 'DONE — Exam 6 R&W modules now serve the new questions.');
}

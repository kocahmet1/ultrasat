/**
 * Replace the two Math modules of "Exam 5" (Practice Test 5) with the 22+22
 * questions authored in scripts/data/practiceTest5Math.json.
 *
 * Mirrors replaceExam5RW.js: the two Math examModule documents are kept (their
 ...
 *  - Figure items: the SVG in scripts/data/practiceTest5Math-assets/ named by
 ...
 *
 * Usage:
 *   node scripts/validatePracticeTest5Math.js       # run this first; must pass
 *   node scripts/replaceExam5Math.js --dry-run      # show the plan, write nothing
 *   node scripts/replaceExam5Math.js                # back up, create, and repoint
 *   node scripts/replaceExam5Math.js --rollback scripts/backups/<file>.json
 */
...
const EXAM_ID = 'OCxassXDYqNnZTCwv5JV'; // "Exam 5"
const data = require(path.resolve(__dirname, 'data/practiceTest5Math.json'));
const ASSETS_DIR = path.resolve(__dirname, 'data/practiceTest5Math-assets');
...
async function main() {
  ...
  if (!math[3] || !math[4]) throw new Error('Could not find Math modules 3 and 4 on Exam 5');
  ...
  const backupFile = path.resolve(__dirname, `backups/exam5_math_backup_${Date.now()}.json`);
  if (!DRY_RUN) {

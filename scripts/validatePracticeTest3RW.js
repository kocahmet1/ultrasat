/**
 * Validate scripts/data/practiceTest3RW.json — the two R&W replacement modules
 * for Exam 3 — against the production schema and College Board consistency rules.
 *
 * Also confirms embedded HTML tables survive the app's DOMPurify sanitizer.
 *
 * Usage: node scripts/validatePracticeTest3RW.js
 */
...
for (const mod of modules) {
  ...
  questions.forEach((qq, i) => {
    ...
    if (BLANK_REQUIRED_SKILLS.has(qq.subcategory)) {
    ...
    if (isUnderlinedFunctionItem) {
      if (openMarkers !== 1 || closeMarkers !== 1
        ...
        errors.push(`${tag}: function item needs exactly one nonempty underlined span`);
      }
    } else if (openMarkers || closeMarkers) {
      errors.push(`${tag}: underline markers are only allowed in underlined-function items`);
    }

/**
 * Validate scripts/data/practiceTest2Math.json — the two Math replacement
 * modules (moduleNumber 3 and 4) for Exam 2 — against the production schema,
 * the College Board math style contract (docs/CB_Math_Style_Spec.md), and the
 * PT2 blueprint (docs/analysis/PT2_math_blueprint.md).
 *
 ...
 *
 * Checks per FORM (new in PT2): domain quotas, skill quotas, visual quota,
...
const EXPECTED = {
  ...
  // PT2 blueprint: M4 runs 5 hard because Q19's SPR is the measured medium dip.
  difficultyMix: { 3: { easy: 9, medium: 7, hard: 6 }, 4: { easy: 9, medium: 8, hard: 5 } },
  visualsPerModule: 4,
  // Form-level quotas from docs/analysis/PT2_math_blueprint.md
  domainQuota: { Algebra: 16, 'Advanced Math': 13, 'Problem-Solving and Data Analysis': 7, 'Geometry and Trigonometry': 8 },
  skillQuota: {
    'linear-equations-one-variable': 3, 'linear-functions': 5, 'linear-equations-two-variables': 3,
    'systems-linear-equations': 3, 'linear-inequalities': 2, 'nonlinear-functions': 6,
    ...
    'inference-statistics': 1, 'evaluating-statistical-claims': 0, 'area-volume': 2,
    'lines-angles-triangles': 3, 'right-triangles-trigonometry': 1, 'circles': 2,
  },
  sprCensus: { integers: 9, fractions: 3, decimals: 0, negatives: 1 },
};
...
for (const mod of data.modules || []) {
  ...
  (mod.questions || []).forEach((q, idx) => {
    ...
    const STOP = new Set([
      ...
      'produced', 'summarizes', 'total', 'totals',

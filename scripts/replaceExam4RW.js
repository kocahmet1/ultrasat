/**
 * Replace the two Reading & Writing modules of "Exam 4" (Practice Test 4) with
 * the 27+27 questions authored in scripts/data/practiceTest4RW.json.
 *
 ...
 *
 * Usage:
 *   node scripts/replaceExam4RW.js --dry-run     # show the plan, write nothing
 *   node scripts/replaceExam4RW.js               # back up, create, and repoint
 *   node scripts/replaceExam4RW.js --rollback scripts/backups/<file>.json
 */
...
async function main() {
  ...
  if (!rw[1] || !rw[2]) throw new Error('Could not find R&W modules 1 and 2 on Exam 4');
  ...
  if (!DRY_RUN) {
    ...
    const bpath = path.join(dir, `exam4_rw_backup_${Date.now()}.json`);
  ...
  console.log(DRY_RUN ? '[DRY RUN] no changes written' : 'DONE — Exam 4 R&W modules now serve the new questions.');
}

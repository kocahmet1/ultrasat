...
const AGGREGATE_DOMAIN_TARGETS = {
  'Information and Ideas': { target: 13, officialMin: 12, officialMax: 14 },
...
for (const mod of modules) {
  ...
  questions.forEach((qq, i) => {
    ...
    // Words-in-Context has two official subtypes: the completion form (blank) and
    // the "As used in the text" form, which never carries a blank. All 26 as-used
    // items in the official bank are literary.
    const isAsUsedVocabulary = qq.subcategory === 'words-in-context'
      && /As used in the text/i.test(qq.text || '');
    if (isAsUsedVocabulary) {
      if (blankCount !== 0) errors.push(`${tag}: as-used vocabulary item must not contain a blank, got ${blankCount}`);
    } else if (BLANK_REQUIRED_SKILLS.has(qq.subcategory) || isQuantitativeCompletion) {

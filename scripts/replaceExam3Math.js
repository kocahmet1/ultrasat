/**
 * Replace the two Math modules of "Exam 3" (Practice Test 3) with the 22+22
 * questions authored in scripts/data/practiceTest3Math.json.
 *
 ...
 * (moduleNumber 1 and 2, already rebuilt by replaceExam3RW.js) are never touched.
 *
 ...
 *  - Figure items: the SVG in scripts/data/practiceTest3Math-assets/ named by
 ...
 *
 * Usage:
 *   node scripts/validatePracticeTest3Math.js      # run this first; must pass
 *   node scripts/replaceExam3Math.js --dry-run     # show the plan, write nothing
 *   node scripts/replaceExam3Math.js               # back up, create, and repoint
 *   node scripts/replaceExam3Math.js --rollback scripts/backups/<file>.json
 */
...
async function main() {
  ...
  const backupFile = path.resolve(__dirname, `backups/exam3_math_backup_${Date.now()}.json`);
  if (!DRY_RUN) {

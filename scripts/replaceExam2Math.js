/**
 * Replace the two Math modules of "Exam 2" (Practice Test 2) with the 22+22
 * questions authored in scripts/data/practiceTest2Math.json.
 *
 ...
 *
 * PT2 addition — the old items are RETIRED, not deleted:
 ...
 *  - Figure items: the SVG in scripts/data/practiceTest2Math-assets/ named by
 ...
 *
 * Usage:
 *   node scripts/validatePracticeTest2Math.js       # run this first; must pass
 *   node scripts/replaceExam2Math.js --dry-run      # show the plan, write nothing
 *   node scripts/replaceExam2Math.js                # back up, create, repoint, retire
 *   node scripts/replaceExam2Math.js --no-retire    # repoint only, leave old docs active
 *   node scripts/replaceExam2Math.js --rollback scripts/backups/<file>.json
 */
...
const EXAM_ID = 'vxxtBSqnVQUPsXu9Xy4B'; // "Exam 2"
const data = require(path.resolve(__dirname, 'data/practiceTest2Math.json'));
const ASSETS_DIR = path.resolve(__dirname, 'data/practiceTest2Math-assets');
const RETIRE_REASON = 'superseded by practiceTest2Math (PT2 math rebuild 2026-08)';
...
async function main() {
  ...
  if (!math[3] || !math[4]) throw new Error('Could not find Math modules 3 and 4 on Exam 2');
  ...
  const backupFile = path.resolve(__dirname, `backups/exam2_math_backup_${Date.now()}.json`);
  if (!DRY_RUN) {

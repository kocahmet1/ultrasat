/**
 * Atomically replace Exam 10's Math modules with practiceTest10Math.json.
 *
 * Safety properties:
 ...
 *
 * Usage:
 *   node scripts/validatePracticeTest10Math.js
 *   node scripts/replaceExam10Math.js --dry-run
 *   node scripts/replaceExam10Math.js
 *   node scripts/replaceExam10Math.js --rollback scripts/backups/<backup>.json
 */
...
const RETIRE_REASON = 'replaced by exam10-math-v1 (Practice Test 10 Math rebuild, 2026-08)';
const ROLLBACK_RETIRE_REASON = 'retired because exam10-math-v1 deployment was rolled back';
...
payloadHasher.update('practiceTest10Math.json\0');
...
async function getExamAndMathModules(db) {
  ...
  if (!Array.isArray(moduleIds) || moduleIds.length !== 4 || new Set(moduleIds).size !== 4) {
    throw new Error(`Exam 10 must reference exactly four distinct modules; found ${Array.isArray(moduleIds) ? `${moduleIds.length}/${new Set(moduleIds).size}` : 'no moduleIds array'}`);
  }
  const moduleSnaps = await Promise.all(moduleIds.map((id) => db.collection('examModules').doc(id).get()));
  const missingModuleIds = moduleSnaps.filter((snap) => !snap.exists).map((snap) => snap.id);
  if (missingModuleIds.length) throw new Error(`Exam 10 references missing module docs: ${missingModuleIds.join(', ')}`);

  const found = { 1: [], 2: [], 3: [], 4: [] };
  for (const snap of moduleSnaps) {
    const moduleData = snap.data();
    if (![1, 2, 3, 4].includes(moduleData.moduleNumber)) {
      throw new Error(`Exam 10 module ${snap.id} has unexpected moduleNumber ${moduleData.moduleNumber}`);
    }
    found[moduleData.moduleNumber].push({ id: snap.id, ref: snap.ref, data: moduleData });
  }
  for (const moduleNumber of [1, 2, 3, 4]) {
    if (found[moduleNumber].length !== 1) {
      throw new Error(`Expected exactly one M${moduleNumber}; found ${found[moduleNumber].length}`);
    }
  }
  const math = { 3: found[3][0], 4: found[4][0] };
  // R&W M1/M2 are out of scope. Their complete documents are backed up,
  // concurrency-guarded inside the transaction, and verified byte-for-byte.
  const untouchedModules = [found[1][0], found[2][0]];
...
async function rollback(db, admin, backupFile) {
  ...
  if (backup.version !== 2 || backup.kind !== 'exam10-math-replacement' || backup.exam?.id !== EXAM_ID) {
    throw new Error('Backup is not a supported Exam 10 Math replacement backup');
  }
  ...
  const untouchedNumbers = (backup.untouchedModules || []).map((module) => module.moduleNumber).sort();
  if (untouchedNumbers.join(',') !== '1,2') {
    throw new Error('Backup must contain the complete untouched R&W M1/M2 documents');
  }
  const examRef = db.collection('practiceExams').doc(EXAM_ID);
...
async function verifyDeployment(db, backup, expectedDocs) {
  ...
  if (!arraysEqual(exam.moduleIds || [], backup.exam.moduleIds || [])) {
    throw new Error('Post-write verification: Exam 10 moduleIds changed');
  }
  if (exam.isPublic !== false) throw new Error('Post-write verification: Exam 10 should be unpublished after QC invalidation');
  ...
  const untouchedNumbers = (backup.untouchedModules || []).map((module) => module.moduleNumber).sort();
  if (untouchedNumbers.join(',') !== '1,2') throw new Error('Post-write verification: untouched R&W M1/M2 backup is incomplete');
  const untouchedSnaps = await db.getAll(...backup.untouchedModules.map((module) => db.collection('examModules').doc(module.id)));
  untouchedSnaps.forEach((snap, index) => {
    if (!snap.exists || stableSerialized(snap.data()) !== JSON.stringify(stableObject(backup.untouchedModules[index].fullDocument))) {
      throw new Error(`Post-write verification: untouched R&W M${backup.untouchedModules[index].moduleNumber} (${backup.untouchedModules[index].id}) changed`);
    }
  });
  console.log(`Verified: 44 replacement questions live in their modules; ${retired} outgoing questions soft-retired.`);
  console.log('Verified: Reading and Writing M1/M2 remain byte-for-byte unchanged.');
  console.log('Exam 10 is unpublished and QC is stale, matching the application safety contract.');
}
...
async function main() {
  ...
  console.log(`Untouched R&W docs M1=${preflight.untouchedModules[0].id}, M2=${preflight.untouchedModules[1].id}`);
  ...
  const backupFile = path.join(backupDir, `exam10_math_backup_${Date.now()}.json`);
  ...
  await db.runTransaction(async (transaction) => {
    ...
    for (const untouched of preflight.untouchedModules) {
      ...
      if (!current || stableSerialized(current) !== stableSerialized(untouched.data)) {
        throw new Error(`Untouched Exam 10 module ${untouched.id} changed after backup; deployment aborted`);
      }
    }
  ...
  console.log(`Rollback command: node scripts/replaceExam10Math.js --rollback "${backupFile}"`);
}
...
main().then(() => process.exit(0)).catch((error) => {
  ...
  if (latestBackupFile) {
    ...
    console.error(`node scripts/replaceExam10Math.js --rollback "${latestBackupFile}"`);
  }
  process.exit(1);
});

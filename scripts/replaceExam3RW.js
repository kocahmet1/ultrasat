/**
 * Replace the two Reading & Writing modules of "Exam 3" (Practice Test 3) with
 * the 27+27 questions authored in scripts/data/practiceTest3RW.json.
 *
 ...
 *
 * Usage:
 *   node scripts/replaceExam3RW.js --dry-run     # show the plan, write nothing
 *   node scripts/replaceExam3RW.js               # back up, create, and repoint
 *   node scripts/replaceExam3RW.js --rollback scripts/backups/<file>.json
 *
 * NOTE: This was originally applied via a REST client. This script is the
 * canonical, rerunnable version for future edits and rollback.
 */
...
const EXAM_ID = '0lo7OoIOnDfmrxRlA8Gh'; // "Exam 3"
const data = require(path.resolve(__dirname, 'data/practiceTest3RW.json'));
...
async function main() {
  ...
  if (!rw[1] || !rw[2]) throw new Error('Could not find R&W modules 1 and 2 on Exam 3');
  ...
  if (!DRY_RUN) {
    ...
    const bpath = path.join(dir, `exam3_rw_backup_${Date.now()}.json`);
  ...
  console.log(DRY_RUN ? '[DRY RUN] no changes written' : 'DONE — Exam 3 R&W modules now serve the new questions.');
}

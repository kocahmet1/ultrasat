/**
 * Replace the two Math modules of "Exam 4" (Practice Test 4) with the 22+22
 * questions authored in scripts/data/practiceTest4Math.json.
 *
 * Mirrors replaceExam4RW.js: the two Math examModule documents are kept (their
 ...
 *  - Figure items: the SVG in scripts/data/practiceTest4Math-assets/ named by
 ...
 *
 * Usage:
 *   node scripts/validatePracticeTest4Math.js       # run this first; must pass
 *   node scripts/replaceExam4Math.js --dry-run      # show the plan, write nothing
 *   node scripts/replaceExam4Math.js                # back up, create, and repoint
 *   node scripts/replaceExam4Math.js --rollback scripts/backups/<file>.json
 */
...
const EXAM_ID = 'maYRpKPacaA4t5vnaGGz'; // "Exam 4"
const data = require(path.resolve(__dirname, 'data/practiceTest4Math.json'));
const ASSETS_DIR = path.resolve(__dirname, 'data/practiceTest4Math-assets');
...
async function main() {
  ...
  if (!math[3] || !math[4]) throw new Error('Could not find Math modules 3 and 4 on Exam 4');
  ...
  const backupFile = path.resolve(__dirname, `backups/exam4_math_backup_${Date.now()}.json`);
  if (!DRY_RUN) {

/**
 * Replace the two Reading & Writing modules of "Exam 5" (Practice Test 5) with
 * the 27+27 questions authored in scripts/data/practiceTest5RW.json.
 *
 ...
 *
 * Usage:
 *   node scripts/replaceExam5RW.js --dry-run     # show the plan, write nothing
 *   node scripts/replaceExam5RW.js               # back up, create, and repoint
 *   node scripts/replaceExam5RW.js --rollback scripts/backups/<file>.json
 */
...
const data = require(path.resolve(__dirname, 'data/practiceTest5RW.json'));
...
async function main() {
  ...
  if (!rw[1] || !rw[2]) throw new Error('Could not find R&W modules 1 and 2 on Exam 5');
  ...
  if (!DRY_RUN) {
    ...
    const bpath = path.join(dir, `exam5_rw_backup_${Date.now()}.json`);
  ...
  console.log(DRY_RUN ? '[DRY RUN] no changes written' : 'DONE — Exam 5 R&W modules now serve the new questions.');
}

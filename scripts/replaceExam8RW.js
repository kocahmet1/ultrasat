 ...
 *
 * Usage:
 *   node scripts/replaceExam8RW.js --dry-run     # show the plan, write nothing
 *   node scripts/replaceExam8RW.js               # back up, create, and repoint
 *   node scripts/replaceExam8RW.js --rollback scripts/backups/<file>.json
 */
...
const EXAM_ID = 'xnKtCv9ci0V35sh7TcFL'; // "Exam 8"
const data = require(path.resolve(__dirname, 'data/practiceTest8RW.json'));
...
async function main() {
  ...
  if (!rw[1] || !rw[2]) throw new Error('Could not find R&W modules 1 and 2 on Exam 8');
  ...
  if (!DRY_RUN) {
    ...
    const bpath = path.join(dir, `exam8_rw_backup_${Date.now()}.json`);
  ...
  console.log(DRY_RUN ? '[DRY RUN] no changes written' : 'DONE — Exam 8 R&W modules now serve the new questions.');
}

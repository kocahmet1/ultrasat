/**
 * Replace the two Reading & Writing modules of "Exam 7" (Practice Test 7) with
 * the 27+27 questions authored in scripts/data/practiceTest7RW.json.
 *
 ...
 *
 * Usage:
 *   node scripts/replaceExam7RW.js --dry-run     # show the plan, write nothing
 *   node scripts/replaceExam7RW.js               # back up, create, and repoint
 *   node scripts/replaceExam7RW.js --rollback scripts/backups/<file>.json
 */
...
const EXAM_ID = 'KxtUBwK850szjDJRwmCW'; // "Exam 7"
const data = require(path.resolve(__dirname, 'data/practiceTest7RW.json'));
...
async function main() {
  ...
  if (!rw[1] || !rw[2]) throw new Error('Could not find R&W modules 1 and 2 on Exam 7');
  ...
  if (!DRY_RUN) {
    ...
    const bpath = path.join(dir, `exam7_rw_backup_${Date.now()}.json`);
  ...
  console.log(DRY_RUN ? '[DRY RUN] no changes written' : 'DONE — Exam 7 R&W modules now serve the new questions.');
}

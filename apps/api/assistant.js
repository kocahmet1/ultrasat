...
const saveChatHistory = async (req, userId, quizId, questionId, newMessages) => {
  try {
    if (!req.db || !req.admin) {
      throw new Error('Firebase not available');
    }

    const messagesToAppend = Array.isArray(newMessages) ? newMessages : [newMessages];
    ...
    if (chatSnap.exists) {
      ...
      await chatRef.update({
        messages: [...existingMessages, ...messagesToAppend],
...
async function fetchQuizQuestionDetailsFromFirestore(req, quizId, questionId) {
  try {
    ...
    // Support both question reference and embedded-question SmartQuiz formats.
...
router.post('/', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const totalTokens = Number(assistantResponseData.usage?.total_tokens) || 0;
    await Promise.all([
      saveChatHistory(req, userId, quizId, questionId, [
        { role: 'user', content: currentMessageContent || '' },
        { role: 'assistant', content: assistantResponseData.message || '' }
      ]),
      totalTokens > 0 ? updateTokenUsage(req, userId, totalTokens) : Promise.resolve()
    ]);

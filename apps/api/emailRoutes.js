...
const authErrorOptions = {
  missingTokenMessage: 'Unauthorized',
  invalidTokenMessage: 'Invalid authentication',
  missingAdminMessage: 'Email service not available - Firebase Admin not initialized',
};

const requireAuthenticatedUser = requireAuth({
  ...authErrorOptions,
  logLabel: 'Email auth error',
});

const requireAdminUser = requireAdmin({
  ...authErrorOptions,
  adminRequiredMessage: 'Admin access required',
  missingDbMessage: 'Email service not available - Firebase Admin not initialized',
  adminCheckFailedMessage: 'Failed to verify admin access',
  authLogLabel: 'Email auth error',
  adminLogLabel: 'Email admin auth error',
});
 ...
 */
router.post('/send-verification', requireAuthenticatedUser, async (req, res) => {
  try {
    ...
    if (!authenticatedEmail || authenticatedEmail.toLowerCase() !== email.toLowerCase()) {
      return res.status(403).json({
        error: 'You can only send verification emails to your own account'
      });
    }
 ...
 */
router.post('/test', requireAuthenticatedUser, requireAdminUser, async (req, res) => {
  try {
    const { email, name } = req.body;

 ...
 * Analyzes wrong answers from a SmartQuiz and identifies concepts that need improvement
 * 
 * Request body:
 * {
 *   quizId: string,            // ID of the SmartQuiz
...
router.post('/analyze-quiz', authenticateUser, async (req, res) => {
  try {
    ...
    const quizRef = admin.firestore().collection('smartQuizzes').doc(quizId);
    ...
    const subcategory =
      ...
      quizData.subcategoryIds?.[0] ||
      wrongQuestions[0].subcategory;

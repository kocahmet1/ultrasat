...
const buildPublicUrl = (pathname) => `${publicBasePath}${pathname}`;

export function register() {
  if ('serviceWorker' in navigator) {
    const publicUrl = new URL(buildPublicUrl('/'), window.location.href);
    if (publicUrl.origin !== window.location.origin) {
      return;
    }

    window.addEventListener('load', () => {
      const swUrl = buildPublicUrl('/sw.js');
...
function showUpdateNotification() {
  ...
  if ('Notification' in window && Notification.permission === 'granted') {
    new Notification('UltraSAT Update Available', {
      ...
      icon: buildPublicUrl('/images/optimized/logo.webp'),
      tag: 'app-update'
    });
  } else {
...
export const cacheManager = {
  ...
  async preloadCritical() {
    const criticalResources = [
      buildPublicUrl('/images/optimized/middle.webp'),
      buildPublicUrl('/images/optimized/aihot.webp'),
      buildPublicUrl('/manifest.json')
    ];

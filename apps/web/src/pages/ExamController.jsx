...
function ExamController() {
  ...
  const [, setCurrentModuleQuestions] = useState([]);
  ...
  // Module metadata
  const [moduleMetadata, setModuleMetadata] = useState(DEFAULT_MODULE_METADATA);
  ...
  useEffect(() => {
    const loadAllModuleQuestions = async () => {
      try {
        ...
        const updatedModuleMetadata = { ...DEFAULT_MODULE_METADATA };
    ...
    if (examResponses.length > 0) {
      ...
      for (let moduleNum = 1; moduleNum <= 4; moduleNum++) {
        ...
        if (questions && questions.length > 0) {
          ...
          for (let index = 0; index < enrichedQuestions.length; index += 1) {
            const question = enrichedQuestions[index];

...
const PaymentSuccess = () => {
  ...
  const waitForMembershipUpdate = useCallback(async (expectedTier, maxRetries = 10) => {
    let retries = 0;
    ...
    setMembershipUpdated(true);
  }, [currentUser, getUserMembership]);

  const verifyPayment = useCallback(async (targetSessionId) => {
    try {
      if (!currentUser) {
      ...
      const response = await fetch(`/api/stripe/verify-session/${targetSessionId}`, {
        headers: {
  ...
  }, [currentUser, waitForMembershipUpdate]);
  ...
  }, [currentUser, sessionId, verifyPayment]);
      ...
      <div className="payment-success">
        <div className="success-container error">
          ...
          <div className="action-buttons">
            <button onClick={() => navigate('/membership/upgrade')} className="retry-button">
              Try Again
            </button>

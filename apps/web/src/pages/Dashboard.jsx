...
function Dashboard() {
  ...
  const recommendedFocusAreas = sortedWeak.slice(0, 4);
  const recommendedPracticeAreas = (sortedWeak.length > 0 ? sortedWeak : sortedModerate).slice(0, 3);

  const getRecommendationSubcategoryId = (subcat) => subcat?.subcategoryId || subcat?.subcategory || subcat?.id;

  const recommendationFeedback = (() => {
    if (recommendedFocusAreas.length > 0) {
      const names = recommendedFocusAreas
        .map((subcat) => getSubcategoryNameById(getRecommendationSubcategoryId(subcat)))
        .filter(Boolean)
        .slice(0, 3);
      return `Your current weak spots are ${names.join(', ')}. Prioritize SmartQuiz sessions in those areas to improve faster.`;
    }

    if (recommendedPracticeAreas.length > 0) {
      return 'You are performing steadily overall. SmartQuiz practice in your moderate areas is the fastest way to push them into the strong range.';
    }

    return 'You are performing strongly across your tracked subcategories. Keep taking SmartQuizzes to maintain accuracy and uncover any weaker edge cases.';
  })();
                        ...
                        <div key={subcatId} className="subcategory-card performance-weak">
                          ...
                          <div className="subcategory-card-actions">
                            <button 
                              ...
                              onClick={() => navigate(`/subcategory-progress/${subcatId}`)}
                            >
                              View Details
                            </button>
          ...
          <div className="recommendations-tab">
            ...
            {(!subcategoryStats || subcategoryStats.length === 0) ? (
              ...
              <>
                ...
                {recommendedFocusAreas.length > 0 && (
                  <div className="recommendation-section">
                    ...
                    <div className="recommendation-cards-grid">
                      {recommendedFocusAreas.map((subcat) => {
                        const skillId = getRecommendationSubcategoryId(subcat);
                        const accuracyRate = subcat.accuracyRate !== undefined ? subcat.accuracyRate.toFixed(0) : 'N/A';
                        return (
                        <div className="subcategory-card recommendation-card" key={`rec-skill-${skillId}`}>
                          <div className="subcategory-card-content">
                            ...
                            <p className="recommendation-text">Recent accuracy: {accuracyRate}%. This is one of your weakest tracked subcategories right now.</p>
                          </div>
                          <div className="subcategory-card-actions">
                            <button 
                              className="modern-button primary-button"
                              onClick={() => navigate(`/subcategory-progress/${skillId}`)}
                            >
                              View Details
                            </button>
                          </div>
                ...
                {recommendedPracticeAreas.length > 0 && (
                  <div className="recommendation-section">
                    <h3 className="recommendation-section-title">Recommended SmartQuiz Practice</h3>
                    <div className="recommendation-cards-grid">
                      {recommendedPracticeAreas.map((subcat) => {
                        const subcategoryId = getRecommendationSubcategoryId(subcat);
                        const subcategoryName = getSubcategoryNameById(subcategoryId);
                        return (
                        <div className="subcategory-card recommendation-card" key={`rec-quiz-${subcategoryId}`}>
                          <div className="subcategory-card-content">
                             <h4 className="subcategory-card-title">{subcategoryName} SmartQuiz</h4>
                            <p className="recommendation-text">Start a fresh SmartQuiz that adapts to your current level in this subcategory.</p>
                          </div>
                          <div className="subcategory-card-actions">
                            <button 
                              className="modern-button primary-button"
                              onClick={() => navigate('/smart-quiz-generator', {
                                state: {
                                  subcategoryId,
                                  accuracyRate: subcat.accuracyRate || 0,
                                }
                              })}
                            >
                              Start SmartQuiz
                ...
                {recommendedFocusAreas.length === 0 && recommendedPracticeAreas.length === 0 && sortedStrong.length > 0 && (
                  <div className="no-data-message card-style-container">
                    <p>Your tracked subcategories are currently strong across the board.</p>
                    <p>Keep practicing with SmartQuiz to maintain performance and surface less frequent weaknesses.</p>
                  </div>
                )}
              </>
            )}

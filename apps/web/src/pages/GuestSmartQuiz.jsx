...
function GuestSmartQuiz() {
  ...
  const quizIds = useMemo(() => idsKey.split(',').filter(Boolean), [idsKey]);
  ...
  useEffect(() => {
    ...
    const validSingle = !meta && quizIds.length > 0 && forceLevel;
    const validMeta = meta && quizIds.length > 0 && forceLevel;
    ...
    const cacheKey = JSON.stringify({ ids: quizIds, difficulty });
    ...
    (async () => {
      try {
        ...
        if (validMeta) {
          ...
          pool = await getPublicQuestionsBySubcategory(quizIds[0], difficulty, 40);
          if (!Array.isArray(pool)) pool = [];
          if (pool.length === 0) {
            pool = await getPublicQuestionsBySubcategory(quizIds[0], null, 40);
            if (!Array.isArray(pool)) pool = [];
          }
        }
  ...
  }, [difficulty, forceLevel, meta, navigate, questionCount, questions.length, quizIds]);

...
const SkillDrillQuiz = () => {
  ...
  const generateNewQuiz = useCallback(async (questionData) => {
    try {
  ...
  }, [currentUser?.uid, skillTag]);
  
  // Generate a new quiz from just the skill tag using direct API calls
  const generateNewQuizFromSkillTag = useCallback(async (skillTagToGenerate) => {
    try {
      ...
      if (!lessonDoc.exists()) {
        ...
        try {
          ...
          throw new Error(`Failed to create lesson for ${skillTagToGenerate}: ${lessonError.message}`);
        }
      }
      
      // Now generate quiz questions for this skill
      console.log('Generating quiz for skill tag:', skillTagToGenerate);
      console.log('Human-readable name:', getHumanReadableSubcategory(skillTagToGenerate));
      
      let quizData;
      
      // Try to get quiz from Firestore first
      try {
        const quizRef = doc(db, 'skillQuizzes', skillTagToGenerate);
        const quizDoc = await getDoc(quizRef);
        ...
        if (quizDoc.exists()) {
          ...
          try {
            console.log('Generating new quiz via direct API call');
            quizData = await generateSkillQuiz({
              subcategory: skillTagToGenerate,
              questionText: `Practice questions for ${getHumanReadableSubcategory(skillTagToGenerate)}`
            });
      ...
      // Update the quiz with the drill ID
      await updateDoc(doc(db, 'skillQuizzes', skillTagToGenerate), {
        drillId: drillRef.id
      });
  ...
  useEffect(() => {
    ...
    loadQuizData();
  }, [currentUser, skillTag, navigate, generateNewQuiz, generateNewQuizFromSkillTag]);

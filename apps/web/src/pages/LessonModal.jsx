...
const LessonModal = ({ skillTag, show, onHide, onStartDrill }) => {
  ...
  const generateLessonContent = useCallback(async (skillTagToGenerate) => {
    if (!skillTagToGenerate) return;
    setLoading(true);
    setError('');
    
    try {
      // Call the direct API to generate the lesson
      console.log(`Generating lesson for skill: ${skillTagToGenerate}`);
  ...
  const fetchLesson = useCallback(async (skillTagToFetch) => {
    try {
      // First try to fetch from Firestore
      const lessonRef = doc(db, 'lessons', skillTagToFetch);
      const lessonDoc = await getDoc(lessonRef);
      ...
      if (lessonDoc.exists()) {
        ...
        await generateLessonContent(skillTagToFetch);
      }
    } catch (err) {
          ...
          onClick={() => {
            ...
            if (onStartDrill) {
              ...
              navigate('/smart-quiz-generator', {
                state: { subcategoryId: normalizedSkillTag }
              });
            }
          }} 

...
function ExamModule({ 
  ...
  const latestInitialQuestionIndexRef = useRef(initialQuestionIndex);
  const latestUserAnswersRef = useRef(userAnswers);
  const timeRemainingRef = useRef(timeRemaining);
  const completeModuleRef = useRef(null);

  latestInitialQuestionIndexRef.current = initialQuestionIndex;
  latestUserAnswersRef.current = userAnswers;
  ...
  useEffect(() => {
    ...
    const initIndex = latestInitialQuestionIndexRef.current || 0;
    const latestUserAnswers = latestUserAnswersRef.current;
    setCurrentQuestion(initIndex);
    if (latestUserAnswers) {
      const initialAns = Array.isArray(latestUserAnswers)
        ? latestUserAnswers[initIndex]
        : latestUserAnswers[initIndex];
  ...
  completeModuleRef.current = completeModule;

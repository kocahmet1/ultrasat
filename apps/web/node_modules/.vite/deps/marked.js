...
var Marked = class {
	...
	walkTokens(tokens, callback) {
		...
		for (const token of tokens) {
			...
			switch (token.type) {
				case "table":
					for (const cell of token.header) values = values.concat(this.walkTokens(cell.tokens, callback));
					for (const row of token.rows) for (const cell of row) values = values.concat(this.walkTokens(cell.tokens, callback));
					break;
				case "list":
					values = values.concat(this.walkTokens(token.items, callback));
					break;
				default: if (this.defaults.extensions && this.defaults.extensions.childTokens && this.defaults.extensions.childTokens[token.type]) this.defaults.extensions.childTokens[token.type].forEach((childTokens) => {
					values = values.concat(this.walkTokens(token[childTokens], callback));
				});
				else if (token.tokens) values = values.concat(this.walkTokens(token.tokens, callback));
			}
		}
		return values;
	}
		...
		args.forEach((pack) => {
			...
			if (pack.hooks) {
				...
				for (const prop in pack.hooks) {
					...
					if (Hooks.passThroughHooks.has(prop)) hooks[prop] = (arg) => {
						if (this.defaults.async) return Promise.resolve(pack.hooks[prop].call(hooks, arg)).then((ret) => {
							return prevHook.call(hooks, ret);
						});
						const ret = pack.hooks[prop].call(hooks, arg);
						return prevHook.call(hooks, ret);
					};
					else hooks[prop] = (...args) => {
